...
<div class="card">
	<div class="card-body">
	    ...
	    <input type="hidden" name="id" value="{{ $lienzo->id }}">
			<fieldset class="mb-3">
				<div class="form-group row">
					<label class="col-form-label col-md-2">Nombre</label>
					<div class="col-md-10">
						<input type="text" class="form-control font-weight-bold" id="nombre" name="nombre" placeholder="Nombre" required value="{{$lienzo->nombre}}">
					</div>
				</div>
						...
						<select class="form-control select" data-fouc="" id="idsParticipantes[]" name="idsParticipantes[]" required multiple>
		                    @foreach($participantes as $item)
			                    ...
			                    @php 
			                    	$encontrado = false;
			                    @endphp                    
			                    @foreach($lienzo->participantes as $participante)
			                        @if($item->id==$participante->idUser)
